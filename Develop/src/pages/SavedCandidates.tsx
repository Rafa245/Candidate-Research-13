...
import { Candidate } from '../interfaces/Candidate.interface'; // Assuming Candidate interface is in types.ts

const SavedCandidates = () => {
  const [savedCandidates, setSavedCandidates] = useState<Candidate[]>([]); // State to store saved candidates

  useEffect(() => {
    const fetchSavedCandidates = () => {
      const storedCandidates = localStorage.getItem('savedCandidates');
      if (storedCandidates) {
        setSavedCandidates(JSON.parse(storedCandidates)); // Parse and set state if available
      }
    };
